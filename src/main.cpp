...
int main() {
    ...
    Food::LoadTexture(); // Tải texture cho mồi
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        if (game.showGameOver && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            if (CheckCollisionPointRec(mousePos, game.playAgainButton)) {
                game.Reset();  // Reset lại game
                game.difficulty = ShowDifficultyMenu();  // Chọn lại độ khó
                game.running = true;
            }
        }
    }

    Food::UnloadTexture(); // Giải phóng texture duy nhất
    CloseWindow();
    return 0;
}
